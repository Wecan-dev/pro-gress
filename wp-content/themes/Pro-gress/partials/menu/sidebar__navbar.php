    <div class="sidebar__navbar my-sidebar active d-none d-lg-block">
      <span></span>
      <div class=" py-4 position-relative">
        <a href="<?php echo get_home_url() ?>" class="brand__sidebar"><img src="<?php echo get_template_directory_uri();?>/assets/img/logo-progress- white.png" alt=""></a>

        <ul class="sidebar__list--menu pb-4 mt-5">
          <li><a href="#!" class="active"><img src="<?php echo get_template_directory_uri();?>/assets/img/icon-home.png" alt="">Dashboard</a></li>
          <li><a href="#!"><img src="<?php echo get_template_directory_uri();?>/assets/img/icon-reports.png" alt="">Reportes</a></li>
          <li><a href="<?php echo get_home_url() ?>workout"><img src="<?php echo get_template_directory_uri();?>/assets/img/icon-dumbell.png" alt="">Workouts</a></li>
          <li><a href="#!"><img src="<?php echo get_template_directory_uri();?>/assets/img/icon-calendar.png" alt="">Calendario</a></li>
        </ul>
        <ul class="sidebar__list--menu py-4">
          <li><a href="<?php echo get_home_url() ?>pro-videos"><img src="<?php echo get_template_directory_uri();?>/assets/img/icon-video.png" alt="">Pro-videos</a></li>
          <li><a href="#!"><img src="<?php echo get_template_directory_uri();?>/assets/img/icon-library.png" alt="">Exercises List</a></li>
        </ul>
      </div>
      <div class="btn-soporte">
        <a href="#">Soporte <i class="fa fa-angle-down" aria-hidden="true"></i></a>
      </div>
    </div>